...
/**
 * This is the model class for table "{{%user_wx_token}}".
 *
 * @property int $id
 ...
 */
class UserWxToken extends UserToken
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $model = UserWxToken::findOne(['user_id' => $userId]);
        return $model;
    }

    static public function createUpdateToken($userId, $token)
    {
        $model = UserWxToken::findOneByUserId($userId);
        if (!$model) {
            $model = new UserWxToken();
        }
